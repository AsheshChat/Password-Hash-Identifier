...
def password_hash(h):
    ...
    checks = {
        "bcrypt": lambda: h.startswith(("$2a$", "$2b$", "$2y$")) and l >= 59,
...
def unix_hash(h):
    ...
    checks = {
        ...
        "bcrypt (Unix)": lambda: h.startswith("$2y$") and l >= 59,
...
def database_hash(h):
    ...
    checks = {
        ...
        "MySQL (>= 4.1)": lambda: l == 41 and h.startswith("*") and is_hex(h[1:]),
    }
...
def raw_hash(h):
    ...
    hash_types = {
        32: lambda: ["LM"] if h.isupper() else ["MD5", "NTLM"],
